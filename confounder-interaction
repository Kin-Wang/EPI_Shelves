...
#偏倚（Bias）：又叫做系统误差（Systematic Error），基本上就是你的研究从设计上到操作上再到分析上有问题。
简单说就是，这种误差是你研究的问题，你得尽量想办法把这些误差减少，甚至理想状态下把它们消除掉。

#混杂（Confounding）：自然存在的，你根本就不可能把它们干掉，它们不是你研究本身所造成的问题，你不可能用任何方式把它们干掉，你只能想办法识别它们并控制它们，以限制它们对你的因果推断造成歪曲的影响。

#效应修饰（Effect modification）：准确来说，流行病学所说的Effect modification是生物意义上的或者是本事在因果理论意义上的第三方，对我们所感兴趣的因果关系产生影响。
如果单纯放到统计学意义上，那我们会用交互作用（Interaction）来指代。所以，很多时候，二者都被当成一个东西。但一定要明白，交互作用必须是三个变量存在。只有两个变量，那只能是主效应。


##然后，我们来看一下怎么确定混杂和交互：

##混杂（Confounding）：判断一个第三方因素到底是不是混杂因素（Confounder），我们基本都知道有这几个判断标准。
第一，混杂因素要和结果（Outcome）有因果关系；
第二，混杂因素要和暴露（Exposure）有关系，但因果关系与否无所谓；
第三，混杂因素绝对不能在暴露和结果的因果关系链条中间，也就是说，绝对不可能出现”暴露导致混杂因素进而导致结果“。
满足这三个条件，那这个第三方因素基本就是混杂因素了，可是有时候我们用这三个条件去判断混杂与否并不容易。
一个很简单很重要的特性是，混杂因素一定会歪曲你本来的因果关系。那我们到底如何知道，因果关系是不是被歪曲了呢？我们可以用关联性测量来看（OR，RR，RD什么的）。

#假设：暴露（自变量）是X，结果（因变量）是Y，第三方因素是Z。你原本的X和Y之间的Risk Ratio（RR）是3（Crude；Marginal）。

你可以用分层的方法来看RR有没有变化（当然，前提是Z本身没有很多类别，最好就只有两个类别，有/无，男/女；如果类别很多，那你就要靠统计软件了）。

#第一种情况：

当Z存在：RRz+ = 2；当Z不存在：RRz- = 2.1；
...
贴个图（来自：https://www.ctspedia.org/wiki/pub/CTSpedia/BiasConfound/0326_2tableCF.JPG）
...
交互作用有一个非常非常重要的前提，无论它的存在还是消失，你本来的因果关系都要是真实存在的因果关系，只是因为这个第三方因素的存在或消失，你的因果关系程度存在差异。

#假设：暴露是X，结果是Y，第三方因素是Z。你原本的X和Y之间的RR是3（Crude）。

你可以用分层的方法来看RR有没有变化（当然，前提是Z本身没有很多类别，最好就只有两个类别，有/无，男/女；如果类别很多，那你就要靠统计软件了；如果是连续性变量，可以用回归分析等方法去看有没有交互作用）。

#第一种情况：

当Z存在：RRz+ = 5；当Z不存在：RRz- = 2；

你发现分层后的RR在程度上不同，RRz+ > Crude RR > RRz-。但是，三个RR的方向都一样，它们都>1，这样你可以说存在量化交互作用（Quantitative Interaction）。
注意注意，Crude RR一定要在二者中间啊，如果都大于Crude RR或都小于Crude RR，基本就要往混杂作用上去看了。

#第二种情况：

当Z存在：RRz+ = 5；当Z不存在：RRz- = 1.01 或者 RRz- = 0.6；

你发现分层后的RR在程度上不同，RRz+ > Crude RR > RRz-。而且，有一个分层RR（RRz-）要么差不多到1了要么方向都变了，直接跨越1到另一边了。
但是，另一个分层RR（RRz+）和Crude RR在同一边，而且要比Crude RR程度强。
这种情况，我们说存在质性交互作用（Qualitative Interaction）。
注意注意，Crude RR一定要在二者中间啊，如果都大于Crude RR或都小于Crude RR，基本就要往混杂作用上去看了。

##总结：分层后的两个RR程度要不同，方向可以相同，也可以不同，但是必须有一个分层RR要比原来的RR（Crude RR）程度要强（远离1），而另一个要接近1甚至可以跨越1。
简单理解就是，Crude RR要在两个分层RR之间，类似于你去用分层变量算出来一个加权后的RR，而这个加权后的RR也是真实的。
只不过，分层以后，某一层的RR程度更强，另一层的RR程度变弱。这个第三方因素并没有扭曲你原本的因果关系。

上几个图（https://www.ctspedia.org/do/view/CTSpedia/InteractionGraph）。
